...
読んでいるページ（画像ページ）を **長押し（約 0.5 秒）** すると、押した位置にハートの演出が出て、そのページが「ここすき！」に登録されます。もう一度長押しすると解除されます。登録・解除時にはトーストが表示されます。
...
- `viewer.toggleFavorite(pageIndex)` でプログラムから切り替えでき、変更時には `favoritesChange` イベント（`{ pageIds: string[] }`）が発火します。現在の一覧は `viewer.getState().favoritePageIds` で参照できます。
- HTML ページは対象外です。

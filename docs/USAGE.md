...
- Ctrl / ⌘ + マウスホイール / トラックパッドのピンチ: ズーム（PC向け）。**カーソル直下の点を中心にズーム**し、見開き時は**カーソルが乗っているページだけ**ズームする。ズーム量はホイールの移動量に比例し、1 イベントあたりの変化は `zoom.step` を上限にクランプされる（マウスホイール 1 ノッチ ≒ `1 + zoom.step` 倍）

...
function renderSymbol(): SVGSVGElement {
  const svg = document.createElementNS(SVG_NS, "svg");
  svg.setAttribute("viewBox", "0 0 112.19 99.01");
  svg.setAttribute("class", "comimi-logo-symbol");

  for (const d of SYMBOL_STROKES) {
    svg.append(path(d, "comimi-logo-stroke"));
  }
  svg.append(
    path(SYMBOL_EAR_LEFT, "comimi-logo-stroke comimi-logo-mimi comimi-logo-mimi-left")
  );
  svg.append(
    path(SYMBOL_EAR_RIGHT, "comimi-logo-stroke comimi-logo-mimi comimi-logo-mimi-right")
  );
  svg.append(path(SYMBOL_BODY, "comimi-logo-body"));

  const eyes = document.createElementNS(SVG_NS, "g");
  eyes.setAttribute("class", "comimi-logo-eyes");
  for (const cx of ["40.17", "71.58"]) {
    const eye = document.createElementNS(SVG_NS, "circle");
    eye.setAttribute("class", "comimi-logo-eye");
...
function renderTypo(): HTMLDivElement {
  const wrap = document.createElement("div");
  wrap.className = "comimi-logo-typo-wrap";

  const svg = document.createElementNS(SVG_NS, "svg");
  svg.setAttribute("viewBox", "0 0 227.74 38.52");
  svg.setAttribute("class", "comimi-logo-typo");
  for (const d of TYPO_LETTERS) {
    svg.append(path(d, "comimi-logo-letter"));
  }
  wrap.append(svg);

  for (const index of [1, 2]) {
    const heart = document.createElement("div");
    heart.className = `comimi-logo-heart comimi-logo-heart-${index}`;
    wrap.append(heart);
  }
  return wrap;
}

export function renderComimiLogo(): HTMLDivElement {
  const logo = document.createElement("div");
  logo.className = "comimi-logo";
  logo.setAttribute("role", "img");
  logo.setAttribute("aria-label", "comimi");
  logo.append(renderSymbol(), renderTypo());
  return logo;
}
